...
tBoatData *pBD=0;
Stream* NMEA0183HandlersDebugStream= 0; // &Serial;
...
void InitNMEA0183Handlers(tNMEA2000 *_NMEA2000, tBoatData *_BoatData) {
  pNMEA2000=_NMEA2000;
  pBD=_BoatData;
    if( pNMEA2000 !=0 ) Serial.println(" pNMEA200 !=0 ");
}
...
void HandleRMC(const tNMEA0183Msg &NMEA0183Msg) {
  Serial.print(" RMC: ");  
  if (pBD==0) return;
  if (NMEA0183ParseRMC_nc(NMEA0183Msg,pBD->GPSTime,pBD->Latitude,pBD->Longitude,pBD->COG,pBD->SOG,
      pBD->DaysSince1970,pBD->Variation)) {

    if (pNMEA2000!=0) {
      tN2kMsg N2kMsg;
      SetN2kMagneticVariation(N2kMsg,0,N2kmagvar_Calc,pBD->DaysSince1970,pBD->Variation);
      Serial.print("Variation: ");
      Serial.print(pBD->Variation);
      pNMEA2000->SendMsg(N2kMsg); 
      SetN2kGNSS(N2kMsg,0,pBD->DaysSince1970,pBD->GPSTime,pBD->Latitude,pBD->Longitude,pBD->Altitude,
      ...
      Serial.print(pBD->Latitude);
      Serial.print(" Longitude= "); 
      Serial.print(pBD->Longitude); 
      pNMEA2000->SendMsg(N2kMsg); 
      SetN2kCOGSOGRapid(N2kMsg,0,N2khr_true,pBD->COG,pBD->SOG);    
      Serial.print(" COG= "); 
      Serial.print(RadToDeg(pBD->COG));
      Serial.print("˚ SOG(m/s)= ");
      Serial.println(pBD->SOG);  
      pNMEA2000->SendMsg(N2kMsg); 
    }
...
void HandleGGA(const tNMEA0183Msg &NMEA0183Msg) {
  Serial.print(" GGA: ");
  if (pBD==0) return;
  bool result = NMEA0183ParseGGA_nc(NMEA0183Msg,pBD->GPSTime,pBD->Latitude,pBD->Longitude,
...
//    Serial.print(result); 
    if (pNMEA2000!=0) {
      ...
      pNMEA2000->SendMsg(N2kMsg); 
      Serial.print(" Latitude= "); 
      Serial.print(pBD->Latitude,5);
      Serial.print(" Longitude= "); 
      Serial.println(pBD->Longitude,5);  
...
void HandleHDT(const tNMEA0183Msg &NMEA0183Msg) {
        Serial.print(" HDT: Magnetic heading°= "); 
  if (pBD==0) return;
  if (NMEA0183ParseHDT_nc(NMEA0183Msg,pBD->TrueHeading)) {
      ...
      SetN2kMagneticHeading(N2kMsg,0,MHeading,0,pBD->Variation);
//      SetN2kPGNTrueHeading(N2kMsg,1,pBD->TrueHeading);
//      NMEA2000.SendMsg(N2kMsg);

      Serial.println(RadToDeg(MHeading));
      pNMEA2000->SendMsg(N2kMsg);
    }
    if (NMEA0183HandlersDebugStream!=0) {
...
//
    if (pNMEA2000!=0) {
      ...
      Serial.println(pBD->WaterTemperature,1);
      pNMEA2000->SendMsg(N2kMsg); 
    }
    if (NMEA0183HandlersDebugStream!=0) {
...
//    Serial.print(result); 
    if (pNMEA2000!=0) {
      ...
      Serial.println(pBD->WaterDepth,1);
      pNMEA2000->SendMsg(N2kMsg); 
    }
    if (NMEA0183HandlersDebugStream!=0) {
...
//    Serial.print(result); 
    if (pNMEA2000!=0) {
      ...
      Serial.print(pBD->WindSpeedM,1);
      Serial.print(" Wind Direction= "); 
      Serial.println(pBD->WindDirectionT,1);      
      pNMEA2000->SendMsg(N2kMsg); 
    }
    if (NMEA0183HandlersDebugStream!=0) {
