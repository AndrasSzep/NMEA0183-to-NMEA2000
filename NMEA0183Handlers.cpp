...
void HandleGLL(const tNMEA0183Msg &NMEA0183Msg) {
  Serial.print(" GLL:");
/*  if (pBD==0) return;
  if (NMEA0183ParseGLL_nc(NMEA0183Msg, pBD->GPSTime,   pBD->Latitude ,   pBD->Longitude, pBD->Status)){
    if (pNMEA2000!=0) {
      tN2kMsg N2kMsg;
...
/*
  if (pBD==0) return;
  if (NMEA0183ParseMTW_nc(NMEA0183Msg,   pBD->WaterTemperature)){
    if (pNMEA2000!=0) {
      tN2kMsg N2kMsg;
...
/*
  if (pBD==0) return;
  double Range=100;
  if (NMEA0183ParseDPT_nc(NMEA0183Msg,   pBD->WaterDepth, pBD->Offset, Range)){
    if (pNMEA2000!=0) {
      tN2kMsg N2kMsg;
...
/*  if (pBD==0) return;
  double Range=100;
  if (NMEA0183ParseMWD_nc(NMEA0183Msg,   pBD->WindDirectionT, pBD->WindDirectionM, pBD->WindSpeedK, pBD->WindSpeedM)){
    if (pNMEA2000!=0) {
      tN2kMsg N2kMsg;
...
/*
  ...
  if (NMEA0183ParseMWV_nc(NMEA0183Msg,   pBD->WindDirectionT, WindReference, pBD->WindSpeedM)){
