...
void handleFileSysFormat() {
	FILESYS.format();
	servOTA.send(200, "text/json", "format complete");
}

String getContentType(String filename){
  if(servOTA.hasArg("download")) return "application/octet-stream";
...
bool handleFileRead(String path){
  ...
  if(FILESYS.exists(pathWithGz) || FILESYS.exists(path))
    ...
    servOTA.streamFile(file, contentType);
    file.close();
...
void handleFileDelete(){
	 if(!servOTA.hasArg("file")) {servOTA.send(500, "text/html", "<meta http-equiv='refresh' content='1;url=/main'>Bad arguments. <a href=/main>Back to list</a>"); return;}
  //if(servOTA.args() == 0) return servOTA.send(500, "text/plain", "BAD ARGS");  
	String path = servOTA.arg("file");
	//String path = servOTA.arg(0);
  Serial.printf_P(PSTR("handleFileDelete: '%s'\r\n"),path.c_str());
  if(path == "/")
    return servOTA.send(500, "text/html", "<meta http-equiv='refresh' content='1;url=/main>Can't delete root directory. <a href=/main>Back to list</a>");
  if(!FILESYS.exists(path))
    return servOTA.send(200, "text/html", "<meta http-equiv='refresh' content='1;url=/main'>File not found. <a href=/main>Back to list</a>");
  FILESYS.remove(path);
  servOTA.send(200, "text/html", "<meta http-equiv='refresh' content='1;url=/main'>File deleted. <a href=/main>Back to list</a>");
  logStr += "Deleted ";
  logStr += path;
  logStr +="\n";
  path = String();
...
void handleFileCreate(){
  if(servOTA.args() == 0)
    return servOTA.send(500, "text/plain", "BAD ARGS");
  String path = servOTA.arg(0);
  Serial.printf_P(PSTR("handleFileCreate: %s\r\n"),path.c_str());
  if(path == "/")
    return servOTA.send(500, "text/plain", "BAD PATH");
  if(FILESYS.exists(path))
    return servOTA.send(500, "text/plain", "FILE EXISTS");
  ...
  if(file)
    ...
    return servOTA.send(500, "text/plain", "CREATE FAILED");
	}
  servOTA.send(200, "text/html", "<meta http-equiv='refresh' content='1;url=/main'>File created. <a href=/main>Back to list</a>");
  logStr += "Created ";
  logStr += path;
  logStr +="\n";
  path = String();
...
void handleMain() {
  ...
  String output = "";
  // check arguments
  if(servOTA.hasArg("mode"))
  {
    bMode = servOTA.arg("mode");
    if(bMode.length() > 0) 
      foundMode = true;
    Serial.printf("Mode %s\n",bMode.c_str());
  }
  if(servOTA.hasArg("dir"))// {servOTA.send(500, "text/plain", "BAD ARGS"); return;}  
		path = servOTA.arg("dir");
	else
		path ="/";
  if(servOTA.hasArg("editBlock"))
  {
    bText = servOTA.arg("editBlock");
    if(bText.length() > 0) 
      foundText = true;
  }
  
  if(servOTA.hasArg("nameSave"))
  {
    bName = servOTA.arg("nameSave");
    if(bName.length() > 0) 
      foundName = true;
    if(!bName.startsWith("/")) bName = "/"+ bName;    // is this needed for LittleFS?   
  }
  
  if(servOTA.hasArg("saveBut"))
  {
    foundSaveBut = true;
  }
  ...
  {  
   ...
   output += edit_script;
   output += "</html>";
  //servOTA.send(200, "text/json", output);
	servOTA.send(200, "text/html", output);
}
